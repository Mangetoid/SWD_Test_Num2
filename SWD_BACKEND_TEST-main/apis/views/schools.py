...
class StudentSubjectsScoreAPIView(APIView):
    ...
    def post(request, *args, **kwargs):
        ...
        if not isinstance(student_first_name, str) or not isinstance(student_last_name, str):
            return Response("Invalid student name", status=status.HTTP_400_BAD_REQUEST)

        if not isinstance(subjects_title, str) or not isinstance(score, (int, float)):
            return Response("Invalid subject title or score", status=status.HTTP_400_BAD_REQUEST)

        score = float(score)
        if not (0 <= score <= 100):
            return Response("Score must be between 0 and 100", status=status.HTTP_400_BAD_REQUEST)

        subject_id = None
        for subject in subjects_context:
            if subject["title"] == subjects_title:
                subject_id = subject["id"]
                break

        if not subject_id:
            return Response("Subject's name not found", status=status.HTTP_400_BAD_REQUEST)
        ...
        if not credit_id:
            return Response("Credit not found for subject", status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            score_entry = StudentSubjectsScore.objects.create(student=student, subjects=subject, score=score)
...
class StudentSubjectsScoreDetailsAPIView(APIView):
    ...
    def get(request, *args, **kwargs):
        ...
        #Edit Sarawuth
        if not student_id:
            return Response("Student ID not provided", status=status.HTTP_400_BAD_REQUEST)
        ...
        for subject_score in subject_scores:
            subject_detail = {
                ...
                "grade": get_grade(subject_score.score)
            }
            subject_details.append(subject_detail)

            total_credits += subject_score.credit
            grade_point_sum += get_grade_point(subject_score.score) * subject_score.credit

        grade_point_average = grade_point_sum / total_credits if total_credits > 0 else 0
        ...
        """
        #Edit example_context_data to context_data
...
class PersonnelDetailsAPIView(APIView):
    # Mapping dictionary for personnel type
    ...
    def get(self, request, *args, **kwargs):
        ...
        if not school_title:
            return Response("School title not provided", status=status.HTTP_400_BAD_REQUEST)

        personnel_details = Personnel.objects.filter(school_class__school__title__iexact=school_title).order_by("personnel_type", "school_class__class_order", "last_name")

        your_result = []
        for index, personnel in enumerate(personnel_details, start=1):
            # Get the personnel type string from the mapping dictionary
            personnel_type_str = self.PERSONNEL_TYPE_MAPPING.get(personnel.personnel_type, '')
            data = f"{index}. school: {personnel.school_class.school.title}, role: {personnel_type_str}, class: {personnel.school_class.class_order}, name: {personnel.first_name} {personnel.last_name}"
...
class SchoolHierarchyAPIView(APIView):
    ...
    def get(request, *args, **kwargs):
        ...
        for data in data_pattern:
            ...
            for class_name, personnel_data in data.items():
                if class_name == 'school':
                    continue
                for teacher_name, personnel_list in personnel_data.items():
                    hierarchy = {
                        "School": school_name,
                        "Class": class_name,
                    ...
                    for personnel in personnel_list:
                        for role, personnel_name in personnel.items():
                            hierarchy["Personnel"].append({
                                role: personnel_name
                            })
                    your_result.append(hierarchy)
    ...
    def get(request, *args, **kwargs):
        ...
        def build_structure(data):
            ...
            for item in data:
                structure = {
                    "title": item["title"],
                    "sub": build_structure(item.get("sub", []))
                }
                result.append(structure)
            return result

        your_result = build_structure(data_pattern)
        return Response(your_result, status=status.HTTP_200_OK)
